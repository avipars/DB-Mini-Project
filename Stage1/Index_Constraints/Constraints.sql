...
-- location condition
CREATE INDEX idx_location_quantity_condition ON Location(Quantity);
-- location quantity
...
CREATE INDEX idx_published_by_book_id ON Published_By(ID);

CREATE INDEX idx_location ON Location(ID);
-- foreign key
...
CREATE INDEX idx_written_in_language_id ON Written_In(ID);
-- foreign key should be indexed

CREATE INDEX idx_type_of_genre_id ON Type_of(Genre_ID);
CREATE INDEX idx_type_of_genre ON Type_of(ID);
